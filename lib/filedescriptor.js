...
class FileDescriptor {
     ...
     */
    readFile(source = this.source, options = undefined) {
        options = this.isBinary() ? null : 'utf-8';
    ...
    writeFile(target, content = this.content) {
        ...
        content = isBinaryPath(target) ? content : content.toString();

        return fx.writeFile(target, content).catch(err => {
    ...
    isBinary() {
        return isBinaryPath(this._source);
    }
