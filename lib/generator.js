...
module.exports = async function generate({ source, target, templates, basePath, logger = console }) {

    FileDescriptor.use(addMetadata);
    FileDescriptor.use(renderMarkdown);
    FileDescriptor.use(buildSidebarLevels);
    FileDescriptor.use(renderHtml);
    ...
    let debug = true; //we want to output the content
    files.forEach(async (file) => {
        file.set('sidebar', sidebar);

        if (debug) {
            logger.info(JSON.stringify(sidebar, null, 4));
            debug = false;
        }
        file.renderMarkdown();
        file.render();
