...
class FileSetError extends Error {
...
class FileSet extends Set {
    ...
    /**
     * Convenience method to filter files
    ...
    async forEachAsync(cb) {
        for (let file of this) {
            try {
                ...
                this.logger.error('FileSet:forEachAsync: %s', error.stack);
                return new FileSetError('Iteration Error', file, error);
            }
        }
